class ImmigrationOfficialRepository {
    ...
    async verifyImmigrationOfficial(userId) {
        const immigrationOfficialData = this.mongo.db('test').collection('user').findOne({userId: userId}, {
            userType: 1,
            userStatus: 1
        });
        if (immigrationOfficialData.userType.toLowerCase() !== 'immigrationOfficial' || immigrationOfficialData.userStatus.toLowerCase() !== 'approved') {
    ...
    async viewUserCovidInformation(healthOfficialId, userId) {
        try {
            ...
            const profile = this.mongo.db('test').collection('user').findOne({userId: userId});
    ...
    async viewTravelerProfile(healthOfficialId, travelerId) {
        try {
            await this.verifyImmigrationOfficial(healthOfficialId);
            const travelerProfile = this.mongo.db('test').collection('user').findOne({userId: travelerId});
