...
class PatientRepository {
  ...
  async verifyOfficial(officialUid) {
    const officialData = await this.mongo
      .db('test')
      .collection('user')
      .findOne({uid: officialUid}, {userType: 1, userStatus: 1});
    console.log('Official data: ', officialData);
    if (officialData === null || officialData === undefined) {
      throw new Error('Not a valid official');
    } else if (
      (officialData.userType.toLowerCase() !== 'immigrationOfficial' ||
        officialData.userStatus.toLowerCase() !== 'approved') &&
      (officialData.userType.toLowerCase() !== 'healthOfficial' ||
        officialData.userStatus.toLowerCase() !== 'approved')
    ) {
      throw new Error('Not a valid official');
    }
  ...
  async addStatusForm(formData) {
    ...
    await this.mongo
      .db('test')
      .collection('user')
      .updateOne({uid: formData.patientUid}, {$set: {covidStatus: formData.covidStatus}});
  ...
  updateStatusForm(formData) {
    ...
    return this.mongo
      .db('test')
      .collection('patientForms')
      .updateOne({_id: mongoId}, {$set: formData});
  }
  ...
  raiseFlag(userId, flagType, flagValue, flaggingUser) {
    return this.mongo
      ...
      .updateOne(
        {uid: userId},
        {$set: {doctorFlagInfo: {isFlagged: flagValue, flaggingUser: flaggingUser}}},
      );
  }
  ...
  fetchPatientsCovidInfo() {
    return this.mongo
      ...
      .collection('user')
      .find(
        {userType: 'patient', userStatus: 'APPROVED'},
        {_id: 0, uid: 1, covidStatus: 1, name: 1},
      )
      .toArray();
  }
  ...
  setWantToBeAssignedToDoctor(userId, requestValue) {
    return this.mongo
      .db('test')
      .collection('user')
      .updateOne({uid: userId}, {$set: {wantToBeAssignedToDoctor: requestValue}});
  }
  ...
  updateContactTracingReport(userId, timeStamp, values) {
    return this.mongo
      .db('test')
      .collection('contactTracing')
      .updateOne({patientUid: userId, timeStamp: timeStamp}, {$set: values});
  }
