class HealthOfficialRepository {
    ...
    async verifyHealthOfficial(userId) {
        const healthOfficialData = this.mongo.db('test').collection('user').findOne({userId: userId}, {
            userType: 1,
            userStatus: 1
        });
        if (healthOfficialData.userType.toLowerCase() !== 'healthOfficer' || healthOfficialData.userStatus.toLowerCase() !== 'approved') {
