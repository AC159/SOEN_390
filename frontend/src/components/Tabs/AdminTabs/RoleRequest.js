...
function RoleRequest(props) {
  ...
  useEffect(() => {
    ...
    axios.get(`admin/${currentUser.user.uid}/pending-health-officials`)
      .then((response) => {
    ...
    axios.get(`admin/${currentUser.user.uid}/pending-immigration-officials`)
      .then((response) => {
  ...
  function renderPendingUserList(userList) {
    ...
    return <div>
      {userList.map((user, index) => <RoleRequestBox
        key={index}
